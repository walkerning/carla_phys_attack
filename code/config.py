...
class Config:
    ...
    from datasets_odgt.bdd100k import BDD100K as datadb
    ...
    train_root_folder = os.path.join(root_dir, 'data')
    train_source = os.path.join(
        root_dir, 'data', 'odformat/train.odgt')

    eval_root_folder = os.path.join(root_dir, 'data')
    eval_source = os.path.join(
        root_dir, 'data', 'odformat/val.odgt')
    eval_json = os.path.join(
        root_dir, 'data', 'MSCOCO/instances_val2014.json')

    filter_gt_ignore_label = True
    train_gt_ignore_label = False

    image_short_size = 720
    image_max_size = 1280
    eval_resize = True
    eval_image_short_size = 720
    eval_image_max_size = 1280
