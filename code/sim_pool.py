...
def _worker_loop(sim_cfg, carla_ts, batch_ind_queue, batch_data_queue, int_ind_queue, int_data_queue, b_timeout, worker_num=0, max_num=1):
    # data worker
    if worker_num >= max_num:
        ...
        worker_num += 1
    print(worker_num)
    print("Start worker process: PID ", os.getpid(), color='cyan')
    simulator = CarlaSimulator(**sim_cfg)
    print("Data worker started for simulator {}".format(simulator.simulator_id), color='cyan')
    while 1:
        # ipdb.set_trace()
        try:
            print('here 3')
            ...
            # print('SIM {} ger from int_ind_queue, len {}'.format(simulator.simulator_id, int_ind_queue.qsize()), color='cyan')
        except queue.Empty:
            try:
                print('here 2')
            ...
            # print('SIM {} ger from batch_ind_queue, len {}'.format(simulator.simulator_id, batch_ind_queue.qsize()),
            #       color='cyan')
    ...
    simulator.cleanup()
    print("Data worker stop for simulator {}".format(simulator.simulator_id), color='cyan')
    sys.exit(0)
...
class SimulatorPool(object):
    def __init__(self, carla_transforms, sim_cfgs, batch_size, batch_poll_timeout=2.0):
        ...
        self.worker_num = 0
        _worker_loop(self.sim_cfgs[0], self.carla_transforms, self.batch_ind_queue, self.batch_data_queue,
                      self.int_ind_queue, self.int_data_queue, batch_poll_timeout, self.worker_num)
    ...
    def get_images(self, ct_lst):
        ...
        for c, t in ct_lst:
            ...
            self.int_data_idx += 1
        # print('>>> get image! int data queue len {}'.format(self.int_data_queue.qsize()))
    ...
    def __iter__(self):
        ...
        self.epoch_end_idx = self._epoch_end_idx()
        # print(self.epoch_delimiters)
    ...
    def _epoch_end_idx(self):
        next_epoch_del = np.where(np.array(self.epoch_delimiters) > self.recv_idx)[0]
        if len(next_epoch_del):
            self.epoch_end_idx = self.epoch_delimiters[next_epoch_del[0]]
            # print('next', next_epoch_del)
            # print(self.epoch_delimiters, self.recv_idx)
        else:
            self.epoch_end_idx = self.send_idx
            # print(self.epoch_delimiters, self.recv_idx)
        return self.epoch_end_idx

    def __next__(self):
        # return a batch of images
        # self.epoch_end_idx = self._epoch_end_idx()
        # print('epoch_end_idx', self.epoch_end_idx)
        # print('self.recv_idx', self.recv_idx)
        end_idx = min(self.recv_idx + self.batch_size, self.epoch_end_idx)
...
if __name__ == "__main__":
    ...
    for i, j in enumerate(sim_pool):
        print('enumerate sim_pool:', i)
    ...
    # import ipdb
    # ipdb.set_trace()
    assert len(batches[-1]) == 15, print(len(batches[-1]))
